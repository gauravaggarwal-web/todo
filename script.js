...
const printList = () => {
	...
	arrayList.forEach((item) => {
        ...
        <span class="has-background-danger is-uppercase is-size-7 has-text-white" style='cursor:pointer' onclick="removeItem('${item}')">
...
const removeItem = (item) => {
	let result = arrayList.filter((it) => it != item)
   arrayList = result
   addToLS()
	printList()
}
...
const getFromLS = () => {
   ...
   tempArray ? (arrayList = JSON.parse(tempArray)) + printList() : null
